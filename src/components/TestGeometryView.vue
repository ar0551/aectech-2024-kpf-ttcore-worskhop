...
// Three js objects
let renderer, camera, scene, controls, container, raycaster
...
function onPointerMove( event ) {
    pointer.x = ( (event.clientX - event.srcElement.offsetLeft) / event.srcElement.width ) * 2 - 1;
    pointer.y = - ( (event.clientY  - event.srcElement.offsetTop) / event.srcElement.height ) * 2 + 1;
}
    ...
    loader.parse(buffer, function (object) {
        ...
        object.traverse((child) => {
            if (child.isMesh) {
                ...
                const wireframe = new THREE.LineSegments(wire, wireMaterial);
        ...
        for ( let i = 0; i < object.children.length; i ++ ) {
            object.children[i].metadata = doc.metadata[i];
        }
...
function selectElement(){
    ...
    scene.traverse((child) => {
        if (!child.isLight && child.isCompute) {
            const intersects = raycaster.intersectObjects( child.children, false );
            if ( intersects.length > 0 ) {
                if ( INTERSECTED != intersects[ 0 ].object ) {
                    ...
                    INTERSECTED.material = new THREE.MeshLambertMaterial( { color: 0x000000 } );
...
function onWindowResize() {
    camera.aspect = (window.innerWidth * widthRatio) / (window.innerHeight * heightRatio);
    camera.updateProjectionMatrix();
    renderer.setSize(window.innerWidth * widthRatio, window.innerHeight * heightRatio);
}
